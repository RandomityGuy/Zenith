...
export class PQServer {
	...
	getMissionList(req: http.IncomingMessage) {
		let urlObject = new url.URL(req.url, 'http://localhost/');
		if (!urlObject.searchParams.has("gameType"))
			return "ARGUMENT gameType";
		let obj = Mission.getMissionList(urlObject.searchParams.get("gameType") as ("Single Player" | "Multiplayer"));
		return obj;
	}
